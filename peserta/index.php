...
else{const saved=q.saved_display_option||'';body=`${imageHtml(q.question_image)}${'ABCDEFGH'.split('').filter(k=>String(q['option_'+k.toLowerCase()]||'').trim()).map(k=>`<label class="opt"><input type="radio" name="current_q" value="${k}" ${saved===k?'checked':''} onchange="selectOption('${k}',${q.id})"><b>${k}.</b> ${esc(q['option_'+k.toLowerCase()])}</label>`).join('')}<div class="saved smallsave">${saved?'Tersimpan':''}</div>`}
